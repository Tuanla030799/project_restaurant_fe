...
const UploadFile = ({
  ...
  useEffect(() => {
    onIsUploadProcessing &&
      onIsUploadProcessing(
        !file || fileProgress.progress != 100 || !isValidFile
      )
  }, [file, fileProgress])
  ...
  return (
    <div className="mx-auto w-[600px] flex flex-col">
          ...
          >
            ...
            <p className="text-lg mb-5 font-semibold px-[110px]">
              <Trans i18nKey="action.click_to_upload">
                <span className={clsx('text-primary-400')}>
            ...
            {supportedFormats && (
              <p className="text-lg mt-[-12px] font-semibold px-[110px]">
                {supportedFormats}
              </p>
            )}
            {otherText && (
              <p className="text-xs text-gray-500 mt-5 text-center px-5">
                {otherText}
              </p>
            )}
          </div>
      ...
      {file && (
        <div
          className={clsx(
            'flex items-start p-4 rounded-lg border bg-white mt-9',
        ...
        >
          <div>
            <HighlightIcon
              icon={File}
              size="sm"
              className={clsx('mr-4', !isValidFile && 'text-red-600')}
            />
          </div>
          {isValidFile ? (
            <div className="grow flex flex-col items-stretch">
              ...
              <p className="text-sm text-gray-500">{bytesToSize(file.size)}</p>
            ...
            <div className="grow flex flex-col items-stretch">
              ...
              <p className="text-sm text-red-600 my-1">{file.name}</p>
            </div>
          )}
        </div>
      )}
